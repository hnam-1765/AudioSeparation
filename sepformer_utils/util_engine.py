...
def model_params_mac_summary(model, input, dummy_input, metrics):
    ...
    # ptflops
    if 'ptflops' in metrics and get_model_complexity_info is not None:
        MACs_ptflops, params_ptflops = get_model_complexity_info(
            model, (input.shape[1],), print_per_layer_stat=False, verbose=False)
    ...
    # thop
    if 'thop' in metrics and profile is not None:
    ...
    # torchinfo
    if 'torchinfo' in metrics and summary_ is not None:
        model_profile = summary_(model, input_size=input.size(), verbose=0)
        logger.info(f"torchinfo: MACs={model_profile.total_mult_adds/1e6:.2f}G, "
                    f"Params={model_profile.total_params/1e6:.2f}M")
