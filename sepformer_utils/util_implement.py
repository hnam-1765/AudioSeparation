...
from .decorators import logger_wraps
...
class BaseFactory:
    ...
    def load_class(self, category: str, lib_check: Type[Any], name: str) -> Type[Any]:
        ...
        try:
            return getattr(lib_check, name) if hasattr(lib_check, name) else getattr(importlib.import_module(f"sepformer_utils.implements.{category}"), name)
